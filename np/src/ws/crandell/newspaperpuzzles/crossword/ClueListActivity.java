...
import ws.crandell.newspaperpuzzles.crossword.io.IO;
...
import ws.crandell.newspaperpuzzles.crossword.puz.Playboard.Clue;
import ws.crandell.newspaperpuzzles.crossword.puz.Playboard.Position;
import ws.crandell.newspaperpuzzles.crossword.puz.Playboard.Word;
import ws.crandell.newspaperpuzzles.crossword.view.ScrollingImageView;
import ws.crandell.newspaperpuzzles.crossword.view.ScrollingImageView.ClickListener;
import ws.crandell.newspaperpuzzles.crossword.view.ScrollingImageView.Point;
...
public class ClueListActivity extends CrosswordActivity {
	...
	public void onCreate(Bundle icicle) {
		...
		this.timer = new ImaginaryTimer(NPPCrosswordsApplication.BOARD.getPuzzle()
				.getTime());
		...
		puz = NPPCrosswordsApplication.BOARD.getPuzzle();
		timer.start();
		setContentView(R.layout.cw_clue_list);

		int keyboardType = "CONDENSED_ARROWS".equals(prefs.getString(
				"keyboardType", "")) ? R.xml.cw_keyboard_dpad : R.xml.cw_keyboard;
		...
		this.imageView.setContextMenuListener(new ClickListener() {
			...
			public void onTap(Point e) {
				Word current = NPPCrosswordsApplication.BOARD.getCurrentWord();
				int newAcross = current.start.across;
				int newDown = current.start.down;
				int box = NPPCrosswordsApplication.RENDERER.findBoxNoScale(e);
				...
				if (!newPos.equals(NPPCrosswordsApplication.BOARD
						.getHighlightLetter())) {
					NPPCrosswordsApplication.BOARD.setHighlightLetter(newPos);
		...
		ts.setIndicator("Across",
				this.getResources().getDrawable(R.drawable.cw_across));
		...
		ts.setIndicator("Down", this.getResources()
				.getDrawable(R.drawable.cw_down));

		ts.setContent(R.id.downList);
		this.tabHost.addTab(ts);

		this.tabHost.setCurrentTab(NPPCrosswordsApplication.BOARD.isAcross() ? 0 : 1);
		...
		across.setAdapter(new ArrayAdapter<Clue>(this,
				android.R.layout.simple_list_item_1, NPPCrosswordsApplication.BOARD
						.getAcrossClues()));
		...
		across.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				NPPCrosswordsApplication.BOARD.jumpTo(arg2, true);
		...
		across.setOnItemSelectedListener(new OnItemSelectedListener() {
			public void onItemSelected(AdapterView<?> arg0, View arg1,
					int arg2, long arg3) {
				if (!NPPCrosswordsApplication.BOARD.isAcross()
						|| (NPPCrosswordsApplication.BOARD.getCurrentClueIndex() != arg2)) {
					NPPCrosswordsApplication.BOARD.jumpTo(arg2, true);
					imageView.scrollTo(0, 0);
					render();
		...
		down.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				NPPCrosswordsApplication.BOARD.jumpTo(arg2, false);
				imageView.scrollTo(0, 0);
				render();
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		Word w = NPPCrosswordsApplication.BOARD.getCurrentWord();
		...
		case KeyEvent.KEYCODE_DPAD_LEFT:
			...
			if (!NPPCrosswordsApplication.BOARD.getHighlightLetter().equals(
					NPPCrosswordsApplication.BOARD.getCurrentWord().start)) {
				NPPCrosswordsApplication.BOARD.previousLetter();
		...
		case KeyEvent.KEYCODE_DPAD_RIGHT:
			...
			if (!NPPCrosswordsApplication.BOARD.getHighlightLetter().equals(last)) {
				NPPCrosswordsApplication.BOARD.nextLetter();
				this.render();
			}
		...
		case KeyEvent.KEYCODE_DEL:
			w = NPPCrosswordsApplication.BOARD.getCurrentWord();
			NPPCrosswordsApplication.BOARD.deleteLetter();

			Position p = NPPCrosswordsApplication.BOARD.getHighlightLetter();

			if (!w.checkInWord(p.across, p.down)) {
				NPPCrosswordsApplication.BOARD.setHighlightLetter(w.start);
			}
		...
		case KeyEvent.KEYCODE_SPACE:
			...
			if (!prefs.getBoolean("spaceChangesDirection", true)) {
				NPPCrosswordsApplication.BOARD.playLetter(' ');

				Position curr = NPPCrosswordsApplication.BOARD.getHighlightLetter();

				if (!NPPCrosswordsApplication.BOARD.getCurrentWord().equals(w)
						|| (NPPCrosswordsApplication.BOARD.getBoxes()[curr.across][curr.down] == null)) {
					NPPCrosswordsApplication.BOARD.setHighlightLetter(last);
				}
		...
		if (PlayActivity.ALPHA.indexOf(c) != -1) {
			NPPCrosswordsApplication.BOARD.playLetter(c);

			Position p = NPPCrosswordsApplication.BOARD.getHighlightLetter();

			if (!NPPCrosswordsApplication.BOARD.getCurrentWord().equals(w)
					|| (NPPCrosswordsApplication.BOARD.getBoxes()[p.across][p.down] == null)) {
				NPPCrosswordsApplication.BOARD.setHighlightLetter(last);
			}
	...
	private void render() {
		...
		this.imageView.setBitmap(NPPCrosswordsApplication.RENDERER.drawWord());
	}
}
