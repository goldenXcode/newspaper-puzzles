...
import static ws.crandell.newspaperpuzzles.crossword.nppcrossword.NPPCrosswordsApplication.BOARD;
import static ws.crandell.newspaperpuzzles.crossword.nppcrossword.NPPCrosswordsApplication.RENDERER;
...
import ws.crandell.newspaperpuzzles.crossword.puz.MovementStrategy;
import ws.crandell.newspaperpuzzles.crossword.puz.Playboard;
...
import ws.crandell.newspaperpuzzles.crossword.view.PlayboardRenderer;
import ws.crandell.newspaperpuzzles.crossword.view.ScrollingImageView;
import ws.crandell.newspaperpuzzles.crossword.view.SeparatedListAdapter;
...
import ws.crandell.newspaperpuzzles.crossword.view.ScrollingImageView.ScaleListener;
...
public class PlayActivity extends CrosswordActivity {
	private static final Logger LOG = Logger.getLogger("ws.crandell.newspaperpuzzles.crossword");
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			if (!prefs.getBoolean("showTimer", false)) {
				System.out.println("tabletish "+NPPCrosswordsApplication.isTabletish(metrics) );
				if (NPPCrosswordsApplication.isLandscape(metrics))  {
					if(NPPCrosswordsApplication.isMiniTabletish(metrics)){
		...
		try {
			...
			setContentView(R.layout.cw_play);
			...
			if (clue.getVisibility() != View.GONE
				...
				clue = (TextView) utils.onActionBarCustom(this,
						R.layout.cw_clue_line_only).findViewById(R.id.clueLine);
			}
		...
		if(android.os.Build.VERSION.SDK_INT > 11 && !NPPCrosswordsApplication.isTabletish(metrics)){
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		if (!puz.isUpdatable()) {
			...
			if (NPPCrosswordsApplication.isTabletish(metrics)) {
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		if (item.getTitle().equals("Letter")) {
			...
			Intent i = new Intent(Intent.ACTION_VIEW,
					Uri.parse("file:///android_asset/cw_playscreen.html"), this,
					HTMLActivity.class);
			this.startActivity(i);
	...
	private Dialog createInfoDialog() {
		...
		dialog.setContentView(R.layout.cw_puzzle_info_dialog);
