...
package ws.crandell.newspaperpuzzles.wordsearch.view;
...
import ws.crandell.newspaperpuzzles.wordsearch.Constants;
import ws.crandell.newspaperpuzzles.wordsearch.model.HighScore;
import ws.crandell.newspaperpuzzles.wordsearch.util.AndroidHttpClient;
import ws.crandell.newspaperpuzzles.wordsearch.util.ConversionUtil;
import ws.crandell.newspaperpuzzles.wordsearch.view.WordDictionaryProvider.Word;
import ws.crandell.newspaperpuzzles.wordsearch.view.controller.TextViewGridController;
import ws.crandell.newspaperpuzzles.wordsearch.view.controller.WordSearchActivityController;
...
public class WordSearchActivity extends Activity {
	class StartHighScoreGlobalTask extends AsyncTask<Integer, Integer, List<HighScore>> {
			...
			if (!this.isCancelled() && pd.isShowing() && pd.getWindow() != null) {
				...
				if (highScores != null) {
					...
					if (highScores.size() == 0) {
						str.append(getString(R.string.ws_no_high_scores));
					} else {
					...
					builder.setTitle(R.string.ws_GLOBAL_HIGH_SCORES);
					builder.setNeutralButton(android.R.string.ok, DIALOG_LISTENER_HIGH_SCORES_SHOW);
					builder.setPositiveButton(R.string.ws_LOCAL, DIALOG_LISTENER_HIGH_SCORES_SHOW);
					builder.show();
				} else {
					final DialogHighScoresGlobalShowListener DIALOG_LISTENER_HIGH_SCORES_SHOW = new DialogHighScoresGlobalShowListener();
					AlertDialog.Builder builder = new AlertDialog.Builder(WordSearchActivity.this);
					builder.setMessage(getString(R.string.ws_SCORE_GLOBAL_ERROR));
					builder.setTitle(R.string.ws_GLOBAL_HIGH_SCORES);
					builder.setNeutralButton(android.R.string.ok, DIALOG_LISTENER_HIGH_SCORES_SHOW);
					builder.show();
				}
		...
		protected void onPreExecute() {
			pd.setMessage(getString(R.string.ws_LOADING));
...
//			Debug.stopMethodTracing();
	...
	class DialogNoWordsCustomListener implements DialogInterface.OnClickListener {
		public void onClick(DialogInterface dialog, int which) {
			switch (which) {
			case DialogInterface.BUTTON_POSITIVE:
				Intent intent = new Intent(Intent.ACTION_EDIT, ws.crandell.newspaperpuzzles.wordsearch.view.WordDictionaryProvider.Word.CONTENT_URI);
				intent.setType(ws.crandell.newspaperpuzzles.wordsearch.view.WordDictionaryProvider.Word.CONTENT_TYPE);
	...
	class DialogIntroListener implements DialogInterface.OnClickListener {
		public void onClick(DialogInterface dialog, int which) {
			...
			case DialogInterface.BUTTON_POSITIVE: {
				...
				sp.edit().putString(getString(R.string.ws_prefs_touch_mode), getString(R.string.ws_TAP)).commit();
				break;
			}
			case DialogInterface.BUTTON_NEUTRAL:
				break;
			case DialogInterface.BUTTON_NEGATIVE: {
				SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(WordSearchActivity.this);
				sp.edit().putString(getString(R.string.ws_prefs_touch_mode), getString(R.string.ws_DRAG)).commit();
				break;
			}
			default:
				break;
	...
	class DialogDonateListener implements DialogInterface.OnClickListener {
		public void onClick(DialogInterface dialog, int which) {
			switch (which) {
			case DialogInterface.BUTTON_POSITIVE: {
				String url = "market://details?id=ws.crandell.donate";
			...
			case DialogInterface.BUTTON_NEUTRAL: {
				String url = "http://www.streetspeech.org";
				Intent i = new Intent(Intent.ACTION_VIEW);
				i.setData(Uri.parse(url));
				startActivity(i);
				break;
...
//			Log.e(LOG_TAG, "tracker failed!");
//		}
		setContentView(R.layout.ws_wordsearch_main);
		control = new WordSearchActivityController(this);
		control.restoreState(savedInstanceState);
		{
			SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(this);
			if (!appVer.equals(sp.getString(Constants.KEY_INTRO_VER, null)) && sp.getString(getString(R.string.ws_prefs_touch_mode), null) == null) {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		this.getMenuInflater().inflate(R.menu.ws_wordsearch_options, menu);
		...
		menu.findItem(R.id.menu_donate).setIcon(R.drawable.ws_love);
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		...
		case DIALOG_ID_GAME_OVER: {
			...
			String msg = this.getString(R.string.ws_SCORE_CONGRATULATIONS).replace("%replaceme", hs.getScore().toString()+" ("+ConversionUtil.formatTime.format(new Date(hs.getTime()))+")");
			if (hs.isHighScore()) {
				msg += this.getString(R.string.ws_SCORE_LOCAL_HIGH).replace("%replaceme", Integer.toString(hs.getRank()+1));
			}
			if (hs.isGlobalError()) {
				msg += this.getString(R.string.ws_SCORE_GLOBAL_ERROR);
			} else {
				String global = "";
				if (hs.isGlobalHighScore()) {
					global = this.getString(R.string.ws_SCORE_GLOBAL_HIGH);
				} else {
					global = this.getString(R.string.ws_SCORE_GLOBAL_PERCENT);
				}
			...
			if (hs.isHighScore() || !hs.isGlobalError()) {
				...
				msg += this.getString(R.string.ws_SCORE_INITIALS);
			} else {
...
//			Debug.startMethodTracing("localHS");
			...
			StringBuilder str = new StringBuilder();
			if (highScores.size() == 0) {
				str.append(this.getString(R.string.ws_no_high_scores));
			} else {
				Collections.sort(highScores);
				for (int index = 0; index < highScores.size(); index++) {
					str.append(Integer.toString(index+1)+": "+highScores.get(index).getName()+" " + highScores.get(index).getScore() + " ( " + ConversionUtil.formatTime.format(new Date(highScores.get(index).getTime())) + " )\n");
