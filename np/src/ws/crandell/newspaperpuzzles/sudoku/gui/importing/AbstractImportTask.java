package ws.crandell.newspaperpuzzles.sudoku.gui.importing;

import ws.crandell.newspaperpuzzles.sudoku.db.SudokuDatabase;
import ws.crandell.newspaperpuzzles.sudoku.db.SudokuImportParams;
import ws.crandell.newspaperpuzzles.sudoku.db.SudokuInvalidFormatException;
import ws.crandell.newspaperpuzzles.sudoku.game.FolderInfo;
import ws.crandell.newspaperpuzzles.sudoku.gui.ImportSudokuActivity;
...
public abstract class AbstractImportTask extends
	...
	protected Boolean doInBackground(Void... params) {
		...
		try {
			...
			setError(mContext.getString(R.string.s_unknown_import_error));
		}
	...
	protected void onPostExecute(Boolean result) {
		if (result) {
			...
			if (mFolderCount == 1) {
				Toast.makeText(mContext, mContext.getString(R.string.s_puzzles_saved, mFolder.name),
						Toast.LENGTH_LONG).show();
			} else if (mFolderCount > 1) {
				Toast.makeText(mContext, mContext.getString(R.string.s_folders_created, mFolderCount),
						Toast.LENGTH_LONG).show();
			}
	...
	private Boolean processImportInternal() {
		...
		try {
			...
			setError(mContext.getString(R.string.s_invalid_format));
		} finally {
		...
		if (mFolderCount == 0 && mGameCount == 0) {
			setError(mContext.getString(R.string.s_no_puzzles_found));
			return false;
		}
