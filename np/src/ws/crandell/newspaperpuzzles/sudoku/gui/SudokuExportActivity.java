...
import ws.crandell.newspaperpuzzles.sudoku.gui.exporting.FileExportTask;
import ws.crandell.newspaperpuzzles.sudoku.gui.exporting.FileExportTaskParams;
import ws.crandell.newspaperpuzzles.sudoku.gui.exporting.FileExportTaskResult;
import ws.crandell.newspaperpuzzles.sudoku.gui.exporting.FileExportTask.OnExportFinishedListener;
...
//	public static final String EXTRA_SUDOKU_ID = "SUDOKU_ID";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.s_sudoku_export);
...
//			mExportParams.sudokuID = intent.getLongExtra(EXTRA_SUDOKU_ID, 0);
		...
		switch (id) {
			case DIALOG_FILE_EXISTS:
				...
				return new AlertDialog.Builder(this)
						.setTitle(R.string.s_export)
						.setMessage(R.string.s_file_exists)
			...
			case DIALOG_PROGRESS:
				...
				progressDialog.setTitle(R.string.s_app_name);
				progressDialog.setMessage(getString(R.string.s_exporting));
	...
	private void exportToFile() {
		...
		if (!sdcard.exists()) {
			Toast.makeText(SudokuExportActivity.this, R.string.s_sdcard_not_found, Toast.LENGTH_LONG);
			finish();
		}
		...
		mFileExportTask.setOnExportFinishedListener(new OnExportFinishedListener() {
			...
			public void onExportFinished(FileExportTaskResult result) {
				...
				if (result.successful) {
					Toast.makeText(SudokuExportActivity.this, getString(R.string.s_puzzles_have_been_exported, result.file), Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(SudokuExportActivity.this, getString(R.string.s_unknown_export_error), Toast.LENGTH_LONG).show();
				}
				finish();
			}
		});
